...
def visualize_codesign(args, exp_name):
    ...
    while(True):
        ...
        for robot_rank in robot_ranks:
            ...
            save_path_controller = os.path.join(EXPERIMENT_PARENT_DIR, exp_name, "generation_" + str(gen_number), "controller1", f'{robot_index}')
...
def visualize_group_ppo(args, exp_name):
    ...
    while True:
        ...
        save_path_controller = os.path.join(exp_dir, job, "controller1", f"{robot}_{env_name}.zip")
        model = PPO.load(save_path_controller)
...
def visualize_ppo(args, exp_name):
    ...
    while True:
        ...
        save_path_controller = os.path.join(exp_dir, "controller1", f"{env_name}.zip")
        model = PPO.load(save_path_controller)
        rollout(env_name, num_iters, model, structure[0], structure[1])
