...
def add_ppo_args(parser: argparse.ArgumentParser) -> None:
    ...
    ppo_parser.add_argument(
        '--verbose-ppo', default=0, type=int, help='Verbosity level for PPO: 0 for no output, 1 for info messages (such as device or wrappers used), 2 for debug messages (default: 0)'
    )
    ppo_parser.add_argument(
    ...
    ppo_parser.add_argument(
        '--total-timesteps', default=6e5, type=int, help='Total number of timesteps for PPO (default: 6e5)'
    )
    ppo_parser.add_argument(
        '--log-interval', default=50, type=int, help='Episodes before logging PPO (default: 50)'
    )
    ppo_parser.add_argument(
        '--n-envs', default=4, type=int, help='Number of parallel environments for PPO (default: 4)'
    )
    ppo_parser.add_argument(
        '--n-eval-envs', default=4, type=int, help='Number of parallel environments for PPO evaluation (default: 4)'
    )
    ppo_parser.add_argument(
        '--n-evals', default=4, type=int, help='Number of times to run the environment during each eval (default: 4)'
    )
    ppo_parser.add_argument(
        '--eval-interval', default=1e4, type=int, help='Number of steps before evaluating PPO model (default: 1e4)'
    )