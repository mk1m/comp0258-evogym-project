...
def run_ga(
    ...
    exp_name, env_name1, env_name2, pop_size, structure_shape, max_evaluations, num_cores = (
        args.exp_name,
        args.env_name_1,
        args.env_name_2,
        args.pop_size,
    ...
    while True:
        ...
        for structure in structures:
            ...
            if structure.is_survivor:
                save_path_controller_part1 = os.path.join("saved_data", exp_name, "generation_" + str(generation), "controller1",
                    f"{structure.label}.zip")
                save_path_controller_part_old1 = os.path.join("saved_data", exp_name, "generation_" + str(generation-1), "controller1",
                    f"{structure.prev_gen_label}.zip")
                save_path_controller_part2 = os.path.join("saved_data", exp_name, "generation_" + str(generation), "controller2",
                    f"{structure.label}.zip")
                save_path_controller_part_old2 = os.path.join("saved_data", exp_name, "generation_" + str(generation-1), "controller2",
                    f"{structure.prev_gen_label}.zip")
                
                print(f'Skipping training for {save_path_controller_part1}.\n')
                print(f'Skipping training for {save_path_controller_part2}.\n')
                try:
                    shutil.copy(save_path_controller_part_old1, save_path_controller_part1)
                except:
                    print(f'Error copying controller for {save_path_controller_part1}.\n')
                try:
                    shutil.copy(save_path_controller_part_old2, save_path_controller_part2)
                except:
